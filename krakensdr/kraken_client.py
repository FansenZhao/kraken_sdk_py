...
class KrakenSDRClient:
    def __init__(self, ip_addr="127.0.0.1", data_port=5000, ctrl_port=5001, num_channels=5, freq_mhz=416.588, gain=None, debug=False, timeout=5.0):
        self.ip_addr = ip_addr
        self.data_port = data_port
        self.ctrl_port = ctrl_port
        self.num_channels = num_channels
        self.freq_hz = int(freq_mhz * 1e6)
        self.gain = gain if gain is not None else [10.0] * num_channels
        self.debug = debug
    ...
    def close(self):
        try:
            if self.data_socket:
                try: self.data_socket.sendall(b"q")
                except: pass
                self.data_socket.close()
            if self.ctrl_socket:
                try: self.ctrl_socket.sendall(b"EXIT" + bytearray(124))
                except: pass
    ...
    def _send_control_command(self, msg_bytes):
        ...
        if reply[:4].decode(errors="ignore") != "FNSD":
            raise RuntimeError(f"Control command failed")
    ...
    def set_if_gain(self, gain_list):
        clipped = [int(round(min(self.valid_gains, key=lambda x: abs(x - g)) * 10)) for g in gain_list]
    ...
    def receive_iq_frame(self):
        ...
        if self.debug: self.iq_header.dump_header()
        payload_size = self.iq_header.cpi_length * self.iq_header.active_ant_chs * 2 * int(self.iq_header.sample_bit_depth / 8)
        if payload_size <= 0: return None
        payload = self._recv_exact(self.data_socket, payload_size)
        return np.frombuffer(payload, dtype=np.complex64).reshape(self.iq_header.active_ant_chs, self.iq_header.cpi_length)
    ...
    def get_data_frame(self, max_retry=20):
        ...
        for _ in range(max_retry):
            ...
            if self.iq_header.frame_type == IQHeader.FRAME_TYPE_DATA:
                return iq
        raise RuntimeError("Failed to get DATA frame")